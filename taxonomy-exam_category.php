...
foreach ($exam_profiles as $post) {
    ...
    if ($year && $month) {
        ...
        $exam_profiles_by_year_month[$year][$month][] = [
            ...
            'label' => $fields['label'] ?? '',
            'desc'  => $fields['card_settings']['description_text'] ?? ''
        ];
    }
}
    ...
    <div class="container my-5">
        ...
        <div class="exam-year-section" id="year-<?= esc_attr($year) ?>" style="display: none;">
            <div class="row">
                <?php
                    ksort($months);
                    foreach ($months as $profiles) {
                        foreach ($profiles as $profile): ?>
                <div class="col-md-3 mb-4">
                    <a href="<?= esc_url($profile['link']) ?>" class="text-decoration-none">
                        <div class="card h-100">
                            <?php if ($profile['image']): ?>
                            <img src="<?= esc_url($profile['image']) ?>" class="card-img-top"
                                alt="<?= esc_attr($profile['title']) ?>">
                            <?php endif; ?>
                            <div class="card-body">
                                <h5 class="card-title"><?= esc_html($profile['label'] ?: $profile['title']) ?></h5>
                                <p class="card-text"><?= esc_html($profile['desc']) ?></p>
                            </div>
                        </div>
                    </a>
                </div>
